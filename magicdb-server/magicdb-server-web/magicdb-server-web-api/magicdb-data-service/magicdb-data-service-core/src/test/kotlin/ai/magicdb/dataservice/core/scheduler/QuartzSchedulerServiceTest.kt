...
class QuartzSchedulerServiceTest {
    ...
    fun testStartTask() {
        ...
        // 设置模拟对象的行为
        `when`(taskRepository.getTask(taskId)).thenReturn(task)
        `when`(taskRepository.updateTaskStatus(taskId, ScheduledTaskStatus.PENDING)).thenReturn(true)
        `when`(taskRepository.updateTask(any())).thenReturn(true)
    ...
    fun testPauseTask() {
        ...
        // 设置模拟对象的行为
        `when`(taskRepository.updateTaskStatus(taskId, ScheduledTaskStatus.PAUSED)).thenReturn(true)
        ...
        verify(taskRepository).updateTaskStatus(taskId, ScheduledTaskStatus.PAUSED)
    }
    ...
    fun testResumeTask() {
        ...
        verify(taskRepository).updateTaskStatus(taskId, ScheduledTaskStatus.PENDING)
    }
