...
package ai.magicdb.server.web.api.controller.config;

import java.util.Objects;

import ai.magicdb.server.domain.api.enums.AiSqlSourceEnum;
import ai.magicdb.server.domain.api.model.AIConfig;
...
import ai.magicdb.server.web.api.controller.config.request.AIConfigCreateRequest;
import ai.magicdb.server.web.api.controller.config.request.SystemConfigRequest;
...
public class ConfigController {
    ...
    public ActionResult addChatGptSystemConfig(@RequestBody AIConfigCreateRequest request) {
        ...
        if (Objects.isNull(aiSqlSourceEnum)) {
            sqlSource = AiSqlSourceEnum.MAGICDBAI.getCode();
            aiSqlSourceEnum = AiSqlSourceEnum.MAGICDBAI;
        }
    ...
    /**
     * save magicdb ai config
     *
     * @param request
     */
    private void saveMagicdbAIConfig(AIConfigCreateRequest request) {
        SystemConfigParam param = SystemConfigParam.builder().code(MagicdbAIClient.MAGICDB_OPENAI_KEY).content(
            request.getApiKey()).build();
        configService.createOrUpdate(param);
        SystemConfigParam hostParam = SystemConfigParam.builder().code(MagicdbAIClient.MAGICDB_OPENAI_HOST).content(
            request.getApiHost()).build();
        configService.createOrUpdate(hostParam);
        SystemConfigParam modelParam = SystemConfigParam.builder().code(MagicdbAIClient.MAGICDB_OPENAI_MODEL).content(
    ...
    public DataResult<AIConfig> getChatAiSystemConfig(String aiSqlSource) {
        ...
        if (Objects.isNull(aiSqlSourceEnum)) {
            aiSqlSource = AiSqlSourceEnum.MAGICDBAI.getCode();
        ...
        switch (Objects.requireNonNull(aiSqlSourceEnum)) {
            case OPENAI:
                ...
                break;
            case MAGICDBAI:
                DataResult<Config> magicdbApiKey = configService.find(MagicdbAIClient.MAGICDB_OPENAI_KEY);
                DataResult<Config> magicdbApiHost = configService.find(MagicdbAIClient.MAGICDB_OPENAI_HOST);
                DataResult<Config> magicdbModel = configService.find(MagicdbAIClient.MAGICDB_OPENAI_MODEL);
                config.setApiKey(Objects.nonNull(magicdbApiKey.getData()) ? magicdbApiKey.getData().getContent() : "");
                config.setApiHost(
                    Objects.nonNull(magicdbApiHost.getData()) ? magicdbApiHost.getData().getContent() : "");
                config.setModel(Objects.nonNull(magicdbModel.getData()) ? magicdbModel.getData().getContent() : "");
                break;
            case AZUREAI:
