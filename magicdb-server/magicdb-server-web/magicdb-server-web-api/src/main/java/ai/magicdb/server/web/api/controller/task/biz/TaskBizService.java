package ai.magicdb.server.web.api.controller.task.biz;

import ai.magicdb.server.domain.api.enums.*;
...
import ai.magicdb.server.web.api.controller.rdb.RdbDmlExportController;
import ai.magicdb.server.web.api.controller.rdb.converter.RdbWebConverter;
import ai.magicdb.server.web.api.controller.rdb.doc.DatabaseExportService;
import ai.magicdb.server.web.api.controller.rdb.doc.conf.ExportOptions;
import ai.magicdb.server.web.api.controller.rdb.factory.ExportServiceFactory;
import ai.magicdb.server.web.api.controller.rdb.request.DataExportRequest;
import ai.magicdb.server.web.api.controller.rdb.vo.TableVO;
...
public class TaskBizService {
    ...
    public DataResult<Long> exportResultData(DataExportRequest request) {
        ...
        DbType dbType = JdbcUtils.parse2DruidDbType(MagicDBContext.getConnectInfo().getDbType());
        ...
        ConnectInfo connectInfo = MagicDBContext.getConnectInfo().copy();
    ...
    private void doExportCsv(String sql, File file) {
        ...
        try {
            ...
            SQLExecutor.getInstance().execute(MagicDBContext.getConnection(), sql, headerList -> {
    ...
    private void doExportInsert(String sql, File file, DbType dbType,
        ...
        try (PrintWriter printWriter = new PrintWriter(file, StandardCharsets.UTF_8.name())) {
            ...
            String databaseName = MagicDBContext.getConnectInfo().getDatabaseName();
            String schemaName = MagicDBContext.getConnectInfo().getSchemaName();
            List<String> headerColumns = Lists.newArrayList();
            SQLExecutor.getInstance().execute(MagicDBContext.getConnection(), sql,
                    headerList -> {
