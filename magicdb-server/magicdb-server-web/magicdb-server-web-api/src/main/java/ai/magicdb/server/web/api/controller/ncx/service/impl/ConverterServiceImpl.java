package ai.magicdb.server.web.api.controller.ncx.service.impl;
...
import ai.magicdb.server.domain.repository.mapper.ChartMapper;
...
import ai.magicdb.server.web.api.controller.ncx.dbeaver.DefaultValueEncryptor;
import ai.magicdb.server.web.api.controller.ncx.enums.DataBaseType;
import ai.magicdb.server.web.api.controller.ncx.enums.ExportConstants;
import ai.magicdb.server.web.api.controller.ncx.enums.VersionEnum;
import ai.magicdb.server.web.api.controller.ncx.factory.CipherFactory;
import ai.magicdb.server.web.api.controller.ncx.service.ConverterService;
import ai.magicdb.server.web.api.controller.ncx.vo.UploadVO;
import ai.magicdb.server.web.api.util.XMLUtils;
...
public class ConverterServiceImpl implements ConverterService {
    ...
    public UploadVO uploadFile(File file) {
        ...
        try {
            ...
            // Get the link imported from navicat and write it into the h2 database of magicdb
    ...
    public void insertDBConfig(List<Map<String, Map<String, String>>> list) {
        for (Map<String, Map<String, String>> map : list) {
            for (Map.Entry<String, Map<String, String>> valueMap : map.entrySet()) {
                ...
                //Insert user id
                dataSourceDO.setUserId(ContextUtils.getUserId());
                //Password is the decrypted ciphertext, and then uses magicdb for encryption.
                DesUtil desUtil = new DesUtil(DesUtil.DES_KEY);
