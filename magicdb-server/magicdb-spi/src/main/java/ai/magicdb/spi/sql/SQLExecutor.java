...
import ai.magicdb.server.tools.base.enums.DataSourceTypeEnum;
...
import ai.magicdb.spi.enums.DataTypeEnum;
import ai.magicdb.spi.enums.SqlTypeEnum;
import ai.magicdb.spi.model.*;
import ai.magicdb.spi.util.JdbcUtils;
import ai.magicdb.spi.util.ResultSetUtils;
import ai.magicdb.spi.util.SqlUtils;
...
//    }
    ...
    private ExecuteResult generateQueryExecuteResult(Statement stmt, boolean limitRowSize, Integer offset,
        ...
        try {
            ...
            int magicdbAutoRowIdIndex = getMagicdbAutoRowIdIndex(headerList);
            // Get data information
            List<List<String>> dataList = generateDataList(rs, col, magicdbAutoRowIdIndex, limitRowSize,
                    offset, count);
    ...
    private List<List<String>> generateDataList(ResultSet rs, int col, int magicdbAutoRowIdIndex,
        ...
        while (rs.next()) {
            ...
            dataList.add(row);
            for (int i = 1; i <= col; i++) {
                if (magicdbAutoRowIdIndex == i) {
                    continue;
                }
                ValueProcessor valueProcessor = MagicDBContext.getMetaData().getValueProcessor();
    ...
    private int getMagicdbAutoRowIdIndex(List<Header> headerList) {
     ...
     */
    public List<ai.magicdb.spi.model.Function> functions(Connection connection, String databaseName,
                                                         String schemaName) {
        try (ResultSet resultSet = connection.getMetaData().getFunctions(databaseName, schemaName, null);) {
            return ResultSetUtils.toObjectList(resultSet, ai.magicdb.spi.model.Function.class);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public List<Type> types(Connection connection) {
        try (ResultSet resultSet = connection.getMetaData().getTypeInfo();) {
            return ResultSetUtils.toObjectList(resultSet, ai.magicdb.spi.model.Type.class);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public List<ExecuteResult> execute(Command command) {
        ...
        // parse sql
        String type = MagicDBContext.getConnectInfo().getDbType();
    ...
    private ExecuteResult executeSQL(String originalSql, DbType dbType, Command param) {
        ...
        if (SqlTypeEnum.SELECT.equals(sqlType) && !SqlUtils.hasPageLimit(originalSql, dbType)) {
            String pageLimit = MagicDBContext.getSqlBuilder().pageLimit(originalSql, offset, pageNo, pageSize);
    ...
    private void addRowNumber(ExecuteResult executeResult, int pageNo, int pageSize) {
        ...
        Header rowNumberHeader = Header.builder()
                ...
                .dataType(DataTypeEnum.MAGICDB_ROW_NUMBER
                        .getCode()).build();
    ...
    private ExecuteResult execute(String sql, Integer offset, Integer count) {
        ...
        try {
            executeResult = SQLExecutor.getInstance().execute(sql, MagicDBContext.getConnection(), true, offset, count);
        } catch (SQLException e) {
