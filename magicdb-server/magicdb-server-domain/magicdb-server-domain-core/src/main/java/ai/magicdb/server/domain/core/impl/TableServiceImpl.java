...
import ai.magicdb.server.domain.api.enums.TableVectorEnum;
import ai.magicdb.server.domain.api.param.*;
...
import ai.magicdb.server.domain.core.converter.PinTableConverter;
import ai.magicdb.server.domain.core.converter.TableConverter;
import ai.magicdb.server.domain.repository.Dbutils;
import ai.magicdb.server.domain.repository.entity.TableCacheDO;
import ai.magicdb.server.domain.repository.entity.TableCacheVersionDO;
import ai.magicdb.server.domain.repository.entity.TableVectorMappingDO;
import ai.magicdb.server.domain.repository.mapper.TableCacheMapper;
import ai.magicdb.server.domain.repository.mapper.TableCacheVersionMapper;
import ai.magicdb.server.domain.repository.mapper.TableVectorMappingMapper;
...
import ai.magicdb.spi.enums.EditStatus;
...
import static ai.magicdb.server.domain.core.cache.CacheKey.getColumnKey;
import static ai.magicdb.server.domain.core.cache.CacheKey.getTableKey;
...
public class TableServiceImpl implements TableService {
    ...
    public DataResult<String> showCreateTable(ShowCreateTableParam param) {
        ...
        String ddl = metaSchema.tableDDL(MagicDBContext.getConnection(), param.getDatabaseName(), param.getSchemaName(), param.getTableName());
        return DataResult.of(ddl);
    }

    @Override
    public ActionResult drop(DropParam param) {
        DBManage metaSchema = MagicDBContext.getDBManage();
        metaSchema.dropTable(MagicDBContext.getConnection(), param.getDatabaseName(), param.getTableSchema(), param.getTableName());
        return ActionResult.isSuccess();
    }

    @Override
    public DataResult<String> createTableExample(String dbType) {
        String sql = MagicDBContext.getDBConfig().getSimpleCreateTable();
        return DataResult.of(sql);
    }

    @Override
    public DataResult<String> alterTableExample(String dbType) {
        String sql = MagicDBContext.getDBConfig().getSimpleAlterTable();
        return DataResult.of(sql);
    }
    ...
    public DataResult<Table> query(TableQueryParam param, TableSelector selector) {
        MetaData metaSchema = MagicDBContext.getMetaData();
        List<Table> tables = metaSchema.tables(MagicDBContext.getConnection(), param.getDatabaseName(), param.getSchemaName(), param.getTableName());
        if (!CollectionUtils.isEmpty(tables)) {
            Table table = tables.get(0);
            table.setIndexList(
                    metaSchema.indexes(MagicDBContext.getConnection(), param.getDatabaseName(), param.getSchemaName(), param.getTableName()));
            table.setColumnList(
                    metaSchema.columns(MagicDBContext.getConnection(), param.getDatabaseName(), param.getSchemaName(), param.getTableName()));
            setPrimaryKey(table);
    ...
    public ListResult<Sql> buildSql(Table oldTable, Table newTable) {
        ...
        SqlBuilder sqlBuilder = MagicDBContext.getSqlBuilder();
...
//    }
    ...
    public List<TableIndex> queryIndexes(TableQueryParam param) {
        MetaData metaSchema = MagicDBContext.getMetaData();
        return metaSchema.indexes(MagicDBContext.getConnection(), param.getDatabaseName(), param.getSchemaName(), param.getTableName());
    ...
    public List<Type> queryTypes(TypeQueryParam param) {
        MetaData metaSchema = MagicDBContext.getMetaData();
        return metaSchema.types(MagicDBContext.getConnection());
    }
