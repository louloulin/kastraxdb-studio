...
import ai.magicdb.server.domain.api.param.datasource.DataSourceCloseParam;
...
import ai.magicdb.server.domain.core.converter.DataSourceConverter;
import ai.magicdb.server.domain.core.converter.EnvironmentConverter;
import ai.magicdb.server.domain.core.util.PermissionUtils;
import ai.magicdb.server.domain.repository.Dbutils;
import ai.magicdb.server.domain.repository.entity.DataSourceAccessDO;
import ai.magicdb.server.domain.repository.entity.DataSourceDO;
import ai.magicdb.server.domain.repository.mapper.DataSourceAccessMapper;
import ai.magicdb.server.domain.repository.mapper.DataSourceCustomMapper;
import ai.magicdb.server.domain.repository.mapper.DataSourceMapper;
...
public class DataSourceServiceImpl implements DataSourceService {
    ...
    public ActionResult preConnect(DataSourcePreConnectParam param) {
        ...
        if (driverConfig == null || !driverConfig.notEmpty()) {
            driverConfig = MagicDBContext.getDefaultDriverConfig(param.getType());
        }
    ...
    public ListResult<Database> connect(Long id) {
        ...
        List<Database> databases = MagicDBContext.getMetaData().databases(MagicDBContext.getConnection());
    ...
    private void fillSupportDatabase(List<DataSource> list) {
        ...
        for (DataSource dataSource:list) {
            ...
            if(StringUtils.isNotBlank(type)) {
                DBConfig config = MagicDBContext.getDBConfig(type);
                if(config != null) {
