...
import ai.magicdb.server.domain.api.service.OperationLogService;
import ai.magicdb.server.domain.api.service.TableService;
import ai.magicdb.server.domain.core.converter.CommandConverter;
import ai.magicdb.server.domain.core.util.MetaNameUtils;
...
public class DlTemplateServiceImpl implements DlTemplateService {
    ...
    public ListResult<ExecuteResult> execute(DlExecuteParam param) {
        CommandExecutor executor = MagicDBContext.getMetaData().getCommandExecutor();
    ...
    public ListResult<ExecuteResult> executeSelectTable(DlExecuteParam param) {
        Command command = commandConverter.param2model(param);
        List<ExecuteResult> results = MagicDBContext.getMetaData().getCommandExecutor().executeSelectTable(command);
        return reBuildHeader(results,param.getSchemaName(),param.getDatabaseName());
    }
    ...
    public DataResult<ExecuteResult> executeUpdate(DlExecuteParam param) {
        ...
        DbType dbType =
                JdbcUtils.parse2DruidDbType(MagicDBContext.getConnectInfo().getDbType());
...
//            connection.commit();
    ...
    public DataResult<Long> count(DlCountParam param) {
        ...
        try {
            executeResult = MagicDBContext.getMetaData().getCommandExecutor().execute(sql, MagicDBContext.getConnection(), true, null, null);
   ...
   @Override
    public DataResult<String> getGroupBySql(GroupByParam param) {
        // - Retrieve the SqlBuilder instance from MagicDBContext
        SqlBuilder sqlBuilder = MagicDBContext.getSqlBuilder();
