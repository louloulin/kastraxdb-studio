...
JRE_TARGET_DIR="magicdb-client/static/jre"
CURRENT_ID="123"

# Clean
echo "Clean"
rm -rf magicdb-client/static
rm -rf magicdb-client/versions
rm -rf magicdb-client/release
...
mkdir -p magicdb-client/versions/99.0.${CURRENT_ID}/static
echo -n 99.0.${CURRENT_ID} > magicdb-client/versions/version
cp magicdb-server/magicdb-server-start/target/magicdb-server-start.jar magicdb-client/versions/99.0.${CURRENT_ID}/static/
...
# Packaged client
yarn run build:main:prod -c.productName=MagicDB-Test -c.extraMetadata.version=99.0.${CURRENT_ID} --mac --arm64
