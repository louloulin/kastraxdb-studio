...
// 桌面端的服务器地址
const desktopServiceUrl = `http://127.0.0.1:${__APP_PORT__ || '10821'}`;
...
request.interceptors.request.use((url, options) => {
  ...
  if (localStorage.getItem('Magicdb')) {
    myOptions.headers.Magicdb = localStorage.getItem('Magicdb');
  }
  return {
    options: myOptions,
  };
...
request.interceptors.response.use(async (response) => {
  ...
  if (__ENV__ === 'desktop') {
    const Magicdb = response.headers.get('Magicdb') || '';
    if (Magicdb) {
      localStorage.setItem('Magicdb', Magicdb);
    }
  }
  const { errorCode } = res;
