...
export const getAiSystemConfig = () => {
  configService.getAiSystemConfig({}).then((res) => {
    setAiConfig(res);
    if (res?.aiSqlSource === AIType.MAGICDBAI && res.apiKey) {
...
export const setAiSystemConfig = (aiConfig) => {
  ...
  if (aiConfig?.aiSqlSource === AIType.MAGICDBAI) {
...
export const fetchRemainingUse = (apiKey)=>{
  ...
  if (!apiKey || currentState.aiConfig.aiSqlSource !== AIType.MAGICDBAI) {
